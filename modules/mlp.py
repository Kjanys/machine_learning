...
class MLP:
    def __init__(self, inputs_count=2, hidden_layers_counts_with_act_funcs=None,
                 outputs_count_with_act_funcs=None):
        if hidden_layers_counts_with_act_funcs is None:
            hidden_layers_counts_with_act_funcs = [
                (2, ActivationFunctions.sigmoid, ActivationFunctions.sigmoid_derivative)]
        if outputs_count_with_act_funcs is None:
        ...
        for i in range(len(self.layers_counts_with_funcs) - 1):
            self.weights.append((np.random.rand(self.layers_counts_with_funcs[i][0],
                                ...
                                np.full((self.layers_counts_with_funcs[i][0],
                                         self.layers_counts_with_funcs[i + 1][0]), 0.5))
